...
export default function InterventionScreen({ onNavigate, onConfirmIntervention, caseStatuses, interventionDone }: Props) {
  ...
  const statusColor = (s: string) => {
    const c: Record<string, string> = {
      Critical: "#991B1B", Watch: "#92400E", Healthy: "#166534", Support: "#1D4ED8", Stressed: "#9A3412",
    };
    return c[s] ?? "#374151";
  };
  ...
  return (
    <div style={{ padding: "28px 32px", overflowY: "auto", flex: 1 }}>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <p style={{ fontSize: 13, color: "#687588", margin: "3px 0 0" }}>Group G-07 · Compare broad and targeted intervention and select the least-disruptive support action. All values simulated.</p>
      </div>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#D84C4C", background: "#FEE2E2", padding: "2px 8px", borderRadius: 4, letterSpacing: "0.04em" }}>PRIORITY: CRITICAL</span>
            ...
            <span style={{ fontSize: 11, fontWeight: 600, color: "#687588", background: "#F8FAFC", padding: "3px 8px", borderRadius: 4, border: "1px solid #E3E8EF", whiteSpace: "nowrap" }}>Officer: {ic.officer}</span>
          </div>
          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ flex: 1, padding: "10px 14px", background: "#FEE2E2", borderRadius: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#991B1B", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.04em" }}>Potentially Exposed</div>
              {ic.affectedBorrowers.map(n => <div key={n} style={{ fontSize: 12, color: "#991B1B" }}>· {n}</div>)}
            </div>
            <div style={{ flex: 1, padding: "10px 14px", background: "#DCFCE7", borderRadius: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#166534", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.04em" }}>Healthy — No Intervention</div>
              {ic.healthyBorrowers.map(n => <div key={n} style={{ fontSize: 12, color: "#166534" }}>✓ {n}</div>)}
            </div>
          </div>
          {!reviewOpen && (
            <button onClick={() => setReviewOpen(true)} style={{ width: "100%", marginTop: 12, padding: "9px 0", background: "#3157D5", color: "white", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
              Review Intervention →
            </button>
          )}
        </div>
        ...
        <div style={{ width: 220, display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ padding: "10px 14px", background: "#F8FAFC", borderRadius: 8, border: "1px solid #F1F5F9" }}>
            ...
            <div style={{ fontSize: 9, color: "#9CA3AF", marginTop: 4, fontStyle: "italic" }}>Synthetic data</div>
          </div>
        </div>
      </div>
        ...
        <div style={{ background: "#FFFFFF", border: "1px solid #E3E8EF", borderRadius: 12, padding: "24px", marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
            {interventionTypes.map(opt => (
              <div key={opt.id} onClick={() => setSelectedOption(opt.id)} style={{
                ...
                <div style={{ fontSize: 12, fontWeight: 700, color: selectedOption === opt.id ? "#3157D5" : "#182230", marginBottom: 4 }}>{opt.label}</div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#182230", marginBottom: 8 }}>
              Compare Responses
              <span style={{ fontSize: 11, fontWeight: 400, color: "#687588", marginLeft: 8 }}>— This is the core of the PS question</span>
            </div>
            ...
            {comparisonMode === "broad" && (
              <div style={{ padding: "9px 14px", background: "#FEE2E2", borderRadius: 7, border: "1px solid #FECACA", fontSize: 12, color: "#991B1B", marginBottom: 12 }}>
                <strong>⚠ Option A — Broad:</strong> Flags or monitors the entire group, potentially penalizing Lata who has no documented exposure. This is the unjustified response the PS warns against.
              </div>
            )}
            {comparisonMode === "targeted" && (
              <div style={{ padding: "9px 14px", background: "#DCFCE7", borderRadius: 7, border: "1px solid #BBF7D0", fontSize: 12, color: "#166534", marginBottom: 12 }}>
                <strong>✓ Option B — Targeted:</strong> Supports Asha, Meera, and Kavita — the members with documented exposure. Lata remains untouched. This is the approach the PS requires.
              </div>
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {/* Before */}
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#687588", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>Before Intervention</div>
              ...
              <div>
                ...
                {comparisonMode === "targeted" && (
                  <div style={{ marginTop: 8, padding: "7px 10px", background: "#DCFCE7", borderRadius: 6, fontSize: 11, color: "#166534", border: "1px solid #BBF7D0" }}>
                    ✓ <strong>Healthy member protected:</strong> Lata is unchanged.
                  </div>
                )}
                {comparisonMode === "broad" && (
                  <div style={{ marginTop: 8, padding: "7px 10px", background: "#FEE2E2", borderRadius: 6, fontSize: 11, color: "#991B1B", border: "1px solid #FECACA" }}>
                    ⚠ <strong>Unnecessary penalty:</strong> Lata is flagged despite having no exposure.
                  </div>
                )}
              </div>
            </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button onClick={handleConfirm} style={{ padding: "10px 28px", background: "#168568", color: "white", border: "none", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
              Confirm Simulated Intervention
            </button>
            <span style={{ fontSize: 11, color: "#9CA3AF", fontStyle: "italic" }}>Simulated workflow only — no real lending actions are applied</span>
          </div>
        </div>
      )}
      ...
      {(confirmed || interventionDone) && (
        <div style={{ background: "#F0FDF4", border: "1.5px solid #BBF7D0", borderRadius: 12, padding: "20px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#168568" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
          ...
          <div style={{ padding: "9px 12px", background: "#FFFFFF", borderRadius: 8, border: "1px solid #BBF7D0", fontSize: 11, color: "#687588", marginBottom: 12 }}>
            This is a simulated workflow for demonstration purposes. No real borrower actions, repayments, or lending decisions have been made.
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => onNavigate("immune-memory")} style={{ padding: "8px 18px", background: "#F5F7FA", border: "1px solid #E3E8EF", borderRadius: 8, fontSize: 12, fontWeight: 600, color: "#3157D5", cursor: "pointer" }}>
              View History →
            </button>
            <button onClick={() => onNavigate("passport")} style={{ padding: "8px 18px", background: "#3157D5", color: "white", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              View Stress Passport →
            </button>
          </div>
        </div>
      )}
